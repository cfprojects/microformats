...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:geo="http://www.w3.org/2003/01/geo/wgs84_pos#" xmlns:review="http://dannyayers.com/xmlns/rev/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:mf="http://suda.co.uk/projects/microformats/mf-templates.xsl?template=" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/dcmitype/" xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:foaf="http://xmlns.com/foaf/0.1/" xmlns:vcard="http://www.w3.org/2006/vcard/ns#" xmlns:tag="http://www.holygoat.co.uk/owl/redwood/0.1/tags/" version="1.0">
...
* tags
-->
<xsl:param name="Source">[hSourceURL]</xsl:param>
...
<xsl:template match="/xhtml:html/xhtml:body">
	<rdf:RDF>
		...
		<xsl:for-each select="//*[contains(concat(' ',normalize-space(@class),' '),' hreview ')]">
			<xsl:if test="    (descendant::*[contains(concat(' ',normalize-space(@class),' '),' summary ')] or descendant::*[contains(concat(' ',normalize-space(@class),' '),' item ')]/descendant::*[contains(concat(' ',normalize-space(@class),' '),' fn ')])    and (not($Anchor) or @id = $Anchor)    ">
...
<!--		<review:Review rdf:about="{$reviewAbout}"> -->
	...
	<xsl:for-each select="descendant::*[contains(concat(' ',normalize-space(@class),' '),' type ')][1]">
	  ...
	  <xsl:choose>
	  	<xsl:when test="      $reviewType = 'product' or      $reviewType = 'business' or      $reviewType = 'event' or      $reviewType = 'person' or      $reviewType = 'place' or      $reviewType = 'website' or      $reviewType = 'url'     ">
	  		<dc:type><xsl:value-of select="$reviewType"/></dc:type>
	  	</xsl:when>
	  	<!-- implied types -->
	  	<xsl:when test="//*[       (contains(concat(' ',normalize-space(@class),' '),' item ') and        contains(concat(' ',normalize-space(@class),' '),' vcard '))      ]/descendant::*[contains(concat(' ',normalize-space(@class),' '),' fn ') and contains(concat(' ',normalize-space(@class),' '),' org ')]">
	  		<dc:type>business</dc:type>
	  	</xsl:when>
	  	<xsl:when test="//*[       (contains(concat(' ',normalize-space(@class),' '),' item ') and        contains(concat(' ',normalize-space(@class),' '),' vcard '))      ]">
	  		<dc:type>person</dc:type>
	  	</xsl:when>
	  	<xsl:when test="//*[       (contains(concat(' ',normalize-space(@class),' '),' item ') and        contains(concat(' ',normalize-space(@class),' '),' vevent '))      ]">
  ...
  <xsl:for-each select="descendant::*[   (contains(concat(' ',normalize-space(@class),' '),' reviewer ') and    contains(concat(' ',normalize-space(@class),' '),' vcard '))  ][1]">
	<review:reviewer>
