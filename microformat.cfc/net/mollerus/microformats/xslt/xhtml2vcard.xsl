<?xml version="1.0"?>
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mf="http://suda.co.uk/projects/microformats/mf-templates.xsl?template=" xmlns:uri="http://www.w3.org/2000/07/uri43/uri.xsl?template=" xmlns:common="http://exslt.org/common" extension-element-prefixes="common" version="1.0">

<xsl:import href="xslt/mf-templates.xsl" />
...
<xsl:template name="properties">
	...
	<xsl:variable name="n-elt" select="   .//*[not(ancestor-or-self::*[local-name() = 'del']) = true() and contains(concat(' ', normalize-space(@class), ' '),' n ')]   or    (   .//*[not(ancestor-or-self::*[local-name() = 'del']) = true() and not(contains(concat(' ', normalize-space(@class), ' '),' n '))]   and   .//*[not(ancestor-or-self::*[local-name() = 'del']) = true() and ancestor::*[contains(concat(' ', normalize-space(@class), ' '),' fn ')] and (contains(concat(' ', normalize-space(@class), ' '),' given-name ') or contains(concat(' ', normalize-space(@class), ' '),' family-name '))]     )     "/>
...
N</xsl:text>
			...
			<xsl:choose>
				<xsl:when test="      (string-length(substring-after(normalize-space($fn-val), &quot; &quot;)) = 1) or      (      (string-length(substring-after(normalize-space($fn-val), &quot; &quot;)) = 2) and       (substring(substring-after(normalize-space($fn-val), &quot; &quot;), 2,1) = &quot;.&quot;)      ) or      (       substring(         normalize-space($fn-val),         string-length(          (substring-before(           normalize-space($fn-val), &quot; &quot;          ))         ),         1       ) = &quot;,&quot;      )      ">
					<xsl:variable name="given-name">
						<xsl:value-of select="substring-after(normalize-space($fn-val), &quot; &quot;)"/>
					</xsl:variable>
					<xsl:choose>
						<xsl:when test="substring(          normalize-space($fn-val),          string-length(           (substring-before(            normalize-space($fn-val), &quot; &quot;           ))          ),          1         ) = &quot;,&quot;">
								<xsl:variable name="family-name">
									<xsl:value-of select="substring(             normalize-space($fn-val),             1,             string-length(              (substring-before(               normalize-space($fn-val), &quot; &quot;              ))             )-1            )"/>
								</xsl:variable>
								<xsl:call-template name="escapeText">
