...
')"/>
		...
		<!--[extension]-->
			...
			<xsl:variable name="classTitles" select="extension:node-set($feedLevelElements)/descendant-or-self::xhtml:*[contains(concat(' ',normalize-space(@class),' '),' feed-title ')]"/>
...
<xsl:template name="h2a:entry">
	...
	<xsl:variable name="entry-base">
		...
		<entry>
			...
			<xsl:variable name="classTitles" select="extension:node-set($entryLevelElements)/descendant-or-self::xhtml:*[contains(concat(' ',normalize-space(@class),' '),' entry-title ')]"/>

			<xsl:variable name="headerTitles" select="extension:node-set($entryLevelElements)/descendant-or-self::xhtml:h1|extension:node-set($entryLevelElements)/descendant-or-self::xhtml:h2|extension:node-set($entryLevelElements)/descendant-or-self::xhtml:h3|extension:node-set($entryLevelElements)/descendant-or-self::xhtml:h4|extension:node-set($entryLevelElements)/descendant-or-self::xhtml:h5|extension:node-set($entryLevelElements)/descendant-or-self::xhtml:h6"/>
				...
				<xsl:when test="extension:node-set($entryLevelElements)/descendant::xhtml:a[contains(concat(' ',normalize-space(translate(@rel,'BOKMAR','bokmar')),' '),' bookmark ')]">
					...
					<xsl:variable name="uri">
						<xsl:call-template name="uri:expand">
							<xsl:with-param name="base">
								<xsl:apply-templates mode="h2a:get-base" select="extension:node-set($entryLevelElements)/descendant::xhtml:a[                                      contains(concat(' ',normalize-space(translate(@rel,'BOKMAR','bokmar')),' '),' bookmark ')][1]">
									<xsl:with-param name="params" select="$params"/>
									<xsl:with-param name="fallback" select="$entry-base"/>
								</xsl:apply-templates>
							</xsl:with-param>
							<xsl:with-param name="there">
								<xsl:value-of select="extension:node-set($entryLevelElements)/descendant::xhtml:a[          contains(concat(' ',normalize-space(translate(@rel,'BOKMAR','bokmar')),' '),' bookmark ')          ][1]/@href"/>
							</xsl:with-param>
						</xsl:call-template>
					</xsl:variable>
					<id><xsl:value-of select="$uri"/></id>
			...
			<xsl:if test="extension:node-set($entryLevelElements)/descendant::xhtml:*[contains(concat(' ',normalize-space(@class),' '),' entry-summary ')]">
				<xsl:variable name="first-summary" select="extension:node-set($entryLevelElements)/descendant::xhtml:*[contains(concat(' ',normalize-space(@class),' '),' entry-summary ')][1]"/>
				<summary type="xhtml">
					<!--[extension]-->
						<!-- 
			...
			<xsl:if test="extension:node-set($entryLevelElements)/descendant::xhtml:*[contains(concat(' ',normalize-space(@class),' '),' entry-content ')]">
				<xsl:variable name="first-content" select="extension:node-set($entryLevelElements)/descendant::xhtml:*[contains(concat(' ',normalize-space(@class),' '),' entry-content ')][1]"/>
				<content type="xhtml">
					<!--[extension]-->
						<!--
...
<xsl:template name="h2a:find-author-filter">
	<xsl:choose>
		...
		<xsl:when test="(local-name() = 'q' or local-name() = 'blockquote')                   and namespace-uri() = 'http://www.w3.org/1999/xhtml' "/>
		<xsl:otherwise>
			<xsl:copy>
	...
	<xsl:choose>
		<xsl:when test="name($context) = 'a' and starts-with($context/@href,'mailto:')">
			...
			<xsl:when test="contains($context/@href,'?')">
				<xsl:value-of select="      substring-before(substring-after($context/@href,'mailto:'), '?')"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:choose>
		<xsl:when test="descendant::xhtml:a[contains(concat(' ',normalize-space(@class),' '),' url ')]">
			<uri>
				<xsl:call-template name="uri:expand">
					<xsl:with-param name="base">
						<xsl:apply-templates mode="h2a:get-base" select="descendant::xhtml:a[                    contains(concat(' ',normalize-space(@class),' '),' url ')]">
							<xsl:with-param name="params" select="$params"/>
		...
		<xsl:when test="descendant::*[contains(concat(' ',normalize-space(@class),' '),' url ')]">
			<uri>
				<xsl:call-template name="uri:expand">
					<xsl:with-param name="base">
						<xsl:apply-templates mode="h2a:get-base" select="descendant::xhtml:*[                                    contains(concat(' ',normalize-space(@class),' '),' url ')]">
							<xsl:with-param name="params" select="$params"/>
							<xsl:with-param name="fallback" select="$vcard-base"/>
						</xsl:apply-templates>
					</xsl:with-param>
					<xsl:with-param name="there">
						<xsl:value-of select="descendant::xhtml:*[                             contains(concat(' ',normalize-space(@class),' '),' url ')]"/>
					</xsl:with-param>
				</xsl:call-template>
			</uri>
		</xsl:when>
...
<xsl:template name="h2a:extract-tag">
	...
	<xsl:if test="$params/h2a:tag-schemes != 0">
		...
		<xsl:variable name="scheme">
			<xsl:value-of select="    substring( $uri-expanded, 1, string-length($uri-expanded) - string-length($basename) )    "/>
		</xsl:variable>
...
<xsl:template name="h2a:decode-uri">
	...
	<xsl:choose>
		...
		<xsl:when test="system-property('xsl:vendor-url') = 'http://xml.apache.org/xalan-j'">
			<xsl:value-of xmlns:xalan="http://xml.apache.org/xalan/java" select="xalan:java.net.URLDecoder.decode($uri,'UTF-8')"/>
		</xsl:when>
		<xsl:when test="system-property('xsl:vendor-url') = 'http://www.saxonica.com/'">
			<xsl:value-of xmlns:saxon="java:java.net.URLDecoder" select="saxon:decode($uri,'UTF-8')"/>
		</xsl:when>
		<xsl:otherwise>
...
<xsl:template name="h2a:plus_to_space">
	...
	<xsl:choose>
		<xsl:when test="contains($string,'+')">
			<xsl:variable name="new_string" select="     concat(      substring-before($string,'+'),      ' ',      substring-after($string,'+')     )     "/>
			<xsl:call-template name="h2a:plus_to_space">
...
-->
<xsl:template mode="h2a:sanitize-html" match="xhtml:a|xhtml:abbr|xhtml:acronym|xhtml:address|xhtml:area| xhtml:b|xhtml:big|xhtml:blockquote|xhtml:br|xhtml:button| xhtml:caption|xhtml:center|xhtml:cite|xhtml:code|xhtml:col|xhtml:colgroup| xhtml:dd|xhtml:del|xhtml:dfn|xhtml:dir|xhtml:div|xhtml:dl|xhtml:dt| xhtml:em| xhtml:fieldset|xhtml:font|xhtml:form| xhtml:h1|xhtml:h2|xhtml:h3|xhtml:h4|xhtml:h5|xhtml:h6|xhtml:hr| xhtml:i|xhtml:img|xhtml:input|xhtml:ins| xhtml:kbd| xhtml:label|xhtml:legend|xhtml:li| xhtml:map|xhtml:menu| xhtml:ol|xhtml:optgroup|xhtml:option| xhtml:p|xhtml:pre|xhtml:q|xhtml:s| xhtml:samp|xhtml:select|xhtml:small|xhtml:span|xhtml:strike|xhtml:strong|xhtml:sub|xhtml:sup| xhtml:table|xhtml:tbody|xhtml:td|xhtml:textarea|xhtml:tfoot|xhtml:th|xhtml:thead|xhtml:tr|xhtml:tt| xhtml:u|xhtml:ul| xhtml:var|@*">  
	...
	-->
		<xsl:for-each select="@abbr|@accept|@accept-charset|@accesskey|@action|@align|@alt|@axis| @border| @cellpadding|@cellspacing|@char|@charoff|@charset|@checked|@cite|@class|@clear| @cols|@colspan|@color|@compact|@coords| @datetime|@dir|@disabled|@enctype| @for|@frame| @headers|@height|@href|@hreflang|@hspace| @id|@ismap| @label|@lang|@longdesc| @maxlength|@media|@method|@multiple| @name| @nohref|@noshade|@nowrap| @prompt| @readonly|@rel|@rev| @rows|@rowspan|@rules| @scope|@selected|@shape|@size| @span|@src|@start|@summary| @tabindex|@target|@title|@type| @usemap| @valign|@value|@vspace| @width| @xml:lang|@xml:base">
			<xsl:copy/>
		</xsl:for-each>
		<xsl:apply-templates mode="h2a:sanitize-html"/>
	</xsl:copy>
	...
	<xsl:choose>
	  ...
	  <xsl:comment>Blank string, expected datetime(Error code: 01)</xsl:comment>
		...
		<xsl:when test="$phase = 'year'">
			...
			<xsl:choose>
				<xsl:when test="                     string-length($s4) = 4                     and                     translate($s4,'0123456789','') = ''     ">
					<xsl:call-template name="h2a:pad-datetime">
		...
		<xsl:when test="$phase = 'month'">
			<xsl:choose>
				<xsl:when test="                     string-length($s2) = 2                     and                     translate($s2,'0123456789','') = ''     ">
					<xsl:call-template name="h2a:pad-datetime">
			...
			<xsl:choose>
				<xsl:when test="                     string-length($s2) = 2                     and                     translate($s2,'0123456789','') = ''     ">
						<xsl:choose>
							...
							<xsl:when test="                       (substring($s,3,1) = 'T' or substring($s,3,1) = ' ')                       and                       string-length($s) != 3                       and                       translate(substring($s,4,1),'0123456789','') = ''        ">
								<xsl:call-template name="h2a:pad-datetime">
